...
- Utility files: camelCase — `browserbase.ts`, `posthog-server.ts`
...
- Always call `revalidatePath` after mutations that affect page data — exception: actions that end in `redirect()` (e.g. `actions/auth.ts`) don't need it, since `redirect()` already forces a fresh render of the destination route
...
**Auth mutations** (`signInWithOAuth`, `signOut`, `exchangeOAuthCode`, etc.) always go through a Server Action using `createAuthActions({ cookies: await cookies() })` from `@insforge/sdk/ssr` — never `createInsforgeServer()` for these. Cookie writes (the access/refresh token cookies) only happen server-side, and `createAuthActions` handles setting/clearing them automatically. See `actions/auth.ts`.
...
| --------------------- | -------------------------------------------- | --------------------------- |
| `cta_clicked`         | Landing page CTA button clicked              | label                       |
| `sign_in_initiated`   | OAuth sign-in button clicked                 | provider                    |
| `sign_in_error`       | OAuth redirect fails to start                | provider                    |
| `sign_in_completed`   | OAuth code exchange succeeds                 | (identified by distinctId)  |
| `sign_in_failed`      | OAuth code exchange fails                    | error                       |
...
The first six events ship with auth/landing (Phase 1, feature 03). The last four are planned for Phases 2–5 and not yet implemented. Do not add more without updating this list first.
...
- `@insforge/sdk` (incl. the `@insforge/sdk/ssr` subpath) — InsForge client
