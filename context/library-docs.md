...
Next.js 16 uses the `instrumentation-client.ts` convention (project root) instead of a `lib/posthog-client.ts` module — it's auto-loaded before any app code runs, so there's no init function to call from `layout.tsx`.
...
posthog.init(process.env.NEXT_PUBLIC_POSTHOG_PROJECT_TOKEN!, {
  api_host: "/ingest", // proxied via next.config.ts rewrites
...
// Singleton — reused across requests in the same long-lived process
...
- Use the `getPostHogClient()` singleton — do **not** call `posthog.shutdown()` on it. `shutdown()` permanently closes the client; calling it on a module-level singleton breaks every subsequent request in the same process. `flushAt: 1` + `flushInterval: 0` already force a synchronous flush per `capture()` call, so no event is lost without `shutdown()`. (Only call `shutdown()` if you construct a fresh, non-singleton `PostHog` instance scoped to a single function call.)
